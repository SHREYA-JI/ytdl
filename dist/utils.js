...
async function updateFile() {
    const binPath = path.join(import.meta.dirname, "../bin/");
    const repos = [
        {
            repo: "yt-dlp/yt-dlp",
    ...
    for (const { repo, versionFile, files } of repos) {
        try {
            ...
            if (localVersion === latestVersion) {
                console.log(`✅ [INFO] The local version (${localVersion}) is up to date.`);
                continue;
            }
            ...
            if (!selectedFile) {
                console.error(
                    `❌ [ERROR] No compatible binary found for the platform ${platform} (${arch}).`
                );
                continue;
            }
            ...
            if (!asset) {
                console.error(`❌ [ERROR] Asset not found for binary: ${suffix}`);
                continue;
            }
            ...
            console.log(`⚠️ [INFO] Downloading new binary version...`);
            ...
            console.log(`✅ [SUCCESS] Updated to version: ${latestVersion}`);
        } catch (error) {
            console.error(
                `❌ [ERROR] Failed to update the repository ${repo}: ${error.message}`
            );
        }
    }
}

export { detectSystemInfo, generateRandomName, getYouTubeID, ensureExecutable, handleFile, getVideoUrl, updateFile };
