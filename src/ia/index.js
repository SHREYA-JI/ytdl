...
export async function imageGenV2(textin, model = 'dalle') {
    ...
    return { url: apiResponse.url, prompt: textin, model };
}

export async function ia(text, model = 'gpt-4o', idChat = false) {
    ...
    AiTempSave[model] = AiTempSave[model] || {};
    AiTempSave[model][idChat] = AiTempSave[model][idChat] || [];
...
export async function textV2(input, model = "gpt-4", idChat = false) {
    if (!input) throw new Error("Falta fornecer um texto.");
    AiTempSave2[model] = AiTempSave2[model] || {};
    AiTempSave2[model][idChat] = AiTempSave2[model][idChat] || [];
...
export async function textV3(input, model = "v3") {
    if (!input) throw new Error("Falta fornecer um texto.");
